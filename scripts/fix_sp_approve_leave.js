...
const DROP_SP = `DROP PROCEDURE IF EXISTS sp_approve_leave`;

// Recreate sp_approve_leave — removed 'is_leave_type' which doesn't exist in attendance_daily
// The table has: is_leave (tinyint), leave_shift_type (enum), but NOT is_leave_type
const CREATE_SP = `
...
proc: BEGIN
    ...
    IF p_action = 'Approved' AND v_current_level = 1 THEN
        IF v_approver_2 IS NOT NULL THEN
            ...
            LEAVE proc;
        ELSE
            -- No level 2 — final approval; fall through to attendance update below
            UPDATE leave_requests SET
                status            = 'Approved',
                approved_by_id    = p_approved_by,
                approved_on       = NOW(),
    ...
    -- Phase 3: Update attendance_daily (is_leave_type removed — column doesn't exist)
    SET v_current_date = v_start_date;
...
async function run() {
    ...
    try {
        await conn.query(DROP_SP);
        console.log('✓ Dropped old sp_approve_leave');

        await conn.query(CREATE_SP);
        console.log('✓ Created new sp_approve_leave (is_leave_type removed)');
        console.log('\nDone! The approval endpoint should now work correctly.');
    } catch (err) {
